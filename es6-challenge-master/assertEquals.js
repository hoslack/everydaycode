...
var deepEquals = function(a, b, trace=''){
  // Replace let
  var typeA = getType(a);
  var typeB = getType(b);
  ...
  if (typeof a === 'object')
    ...
    new Set([...Object.keys(a), ...Object.keys(b)]).forEach(
      function(key){deepEquals(a[key], b[key], trace + buildTrace(a, key))}
    );
}
...
var buildTrace = function(element, key){element instanceof Array ? `[${key}]` : `.${key}`;}
